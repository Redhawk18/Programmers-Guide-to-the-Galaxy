...
```makefile
CC=mpicc
CFLAGS=-Wpedantic -Wall -Wextra -Wshadow -Wsign-conversion -ftrapv -O3 -std=c17 
LDFLAGS=

NAME=project_$(shell basename $(CURDIR)).bin
SRC=.
BUILD_DIR=build
...
##### Nixos
This is a directory flake that will automatically install these on entering, and uninstall them when leaving. Make sure you have direnv setup as well, or use `nix develop`.
```nix
{
  inputs = {
